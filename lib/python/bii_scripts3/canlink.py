...
class _CharList:
    ...
    _inv_list= dict((v,k) for k,v in list(_char_list.items()))
    _access= set(('r', 'w', 'rw'))
    def __init__(self):
        pass
    @classmethod
...
class _TypeList:
    ...
    _inv_list= dict((v,k) for k,v in list(_type_list.items()))
...
def _question(options, returns= None, tests= None):
    ...
    if tests is None:
        ...
        def _my_input():
            ...
            return val
    if returns is None:
        returns= list(range(len(options)))
    max_= len(options)
...
def calc_cidnidsob(p):
    ...
    if needs_w:
        if not _check_exists(p,'out_cob','out_cob', me):
            return
    if ("cid" in p) and ("nid" in p):
        return
    if "nid" in p:
        if (needs_r and "in_sob" in p) and \
           (needs_w and "out_sob" in p):
            return
    if needs_r:
...
def interview(tests= None):
    ...
    if   sel=="cob":
        ...
        raise AssertionError("unexpected sel value:").with_traceback(sel)
