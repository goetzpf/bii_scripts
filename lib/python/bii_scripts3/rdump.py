...
from bii_scripts3.typecheck import *
from bii_scripts3.putil import *
...
def _coltrim(x,indent,h_indent,format_):
    r"""internal function
...
def str_dump_util(mystr,indent,options=None):
    ...
    Here are some examples:
    >>> str_dump_util("ABC",6,Options(format_=REPR,start=False))
    ['ABC']
    >>> str_dump_util("ABC",6,Options(format_=STR_D,start=False))
    ['ABC']
    >>> str_dump_util("ABC",6,Options(format_=STR,start=False))
    ['\n', '      ', 'ABC']
    >>> str_dump_util("ABC",6,Options(format_=STR,start=True))
    ['      ', 'ABC']
    """
    if options is None:
...
def record_dump_util(key_val_list,indent,key_add_indent,val_add_indent,
    ...
    >>> pr(record_dump_util(l,10,8,4,"(mytype",")",Options(format_=REPR,start=False)))
    (mytype 'k1' : 1, 'k2' : 2 )
    >>> pr(record_dump_util(l,10,8,4,"(mytype",")",Options(format_=REPR,start=True)))
    (mytype 'k1' : 1, 'k2' : 2 )
    >>> pr(record_dump_util(l,10,8,4,"(mytype",")",Options(format_=STR,start=False)))
    <BLANKLINE>
              (mytype
                      'k1' : 
                          1,
    ...
    >>> pr(record_dump_util(l,10,8,4,"(mytype",")",Options(format_=STR,start=True)))
              (mytype
                      'k1' : 
                          1,
                      'k2' : 
    ...
    >>> pr(record_dump_util(l,10,8,4,"(mytype",")",Options(format_=STR_D,start=False)))
    <BLANKLINE>
              (mytype
                      'k1' : 1,
                      'k2' : 2
              )
    >>> pr(record_dump_util(l,10,8,4,"(mytype",")",Options(format_=STR_D,start=True)))
              (mytype
                      'k1' : 1,
                      'k2' : 2
              )
    ...
    l= [opn,
        ljoin([inb],\
              list(map(lambda x: [x[0],ldump(x[1],\
...
def default_dumper(val,indent,options=None):
    ...
    >>> pr(default_dumper("ABC",10,Options(format_=REPR,start=False)))
    <OBJ: 'ABC'>
    >>> pr(default_dumper([1,2],10,Options(format_=REPR,start=False)))
    <OBJ: [1, 2]>
    >>> pr(default_dumper([1,2],10,Options(format_=STR,start=False)))
    <BLANKLINE>
              <OBJ: [1, 2]>
    >>> pr(default_dumper([1,2],10,Options(format_=STR,start=True)))
              <OBJ: [1, 2]>
    """
    if options is None:
        options= default_options
...
def ldump(val,indent=0,options=None):
    ...
    >>> pr(ldump("A",10,Options(format_=REPR,start=False)))
    'A'
    >>> pr(ldump([1,2],10,Options(format_=REPR,start=False)))
    [ 1, 2 ]
    >>> pr(ldump("A",10,Options(format_=STR,start=False)))
    <BLANKLINE>
              'A'
    >>> pr(ldump("A",10,Options(format_=STR,start=True)))
              'A'
    """
    if options is None:
        options= default_options
...
class Dumpable:
    ...
    >>> a= Dumpable()
    >>> a.dumper(10,Options(format_=REPR,start=False))
    ['(Dumpable)']
    >>> a.dumper(10,Options(format_=STR,start=False))
    ['\n', '          ', '(Dumpable)']
    >>> print(repr(a))
    (Dumpable)
    >>> print(str(a))
    ...
    @classmethod
    def dumper(cls,indent=0,options=default_options):
    ...
    def __str__(self):
        ...
        return self.dump_string(0,Options(format_=STR,start=True))
    def __repr__(self):
        """returns a simple one-line string representation of the object."""
        return self.dump_string(0,Options(format_=REPR,start=True))
...
def dumpable_dumper(val,indent,options=None):
    ...
    Here are some examples:
    >>> a= Dumpable()
    >>> dumpable_dumper(a,4,Options(format_=REPR,start=False))
    ['(Dumpable)']
    """
    asrt_int_range(indent,min_=0)
    if options is None:
...
class Options(Dumpable):
    ...
    >>> print(Options(format_=STR,indent_inc=1,key_indent=2,start=False))
    (rdump.Options:
        'format' : 
            1,
        'indent_inc' : 
    ...
    >>> print(Options(Options(format_=STR,indent_inc=1,key_indent=2,start=False)))
    (rdump.Options:
        'format' : 
            1,
        'indent_inc' : 
    ...
    differently:
    >>> print(Options(Options(format_=STR,indent_inc=1,key_indent=2,start=False),indent_inc=100))
    (rdump.Options:
        'format' : 
            1,
        'indent_inc' : 
    ...
    def __init__(self,
        ...
        if other is not None:
            ...
            self.format        = _def2(format_,other.format)
            ...
            self.format        = _def2(format_,REPR)
...
def scalar_dumper(val,indent,options=default_options):
    ...
    >>> scalar_dumper([1,2],10,Options(format_=STR,start=False))
    ...
    ["'ABC'"]
    >>> scalar_dumper("ABC",10,Options(format_=STR))
    ['          ', "'ABC'"]
    >>> scalar_dumper("ABC",10,Options(format_=STR,start=False))
    ['\n', '          ', "'ABC'"]
    """
...
def list_dumper(val,indent,options=default_options):
    ...
    >>> print(list2str(list_dumper(a,0,Options(format_=STR))))
    [
        1,
        2,
        3
    ...
    >>> print(list2str(list_dumper(a,0,Options(format_=REPR))))
    [ 1, 2, [ 'A', 'B', 'C' ], 4, 5 ]
    ...
    >>> print(list2str(list_dumper(a,0,Options(format_=STR,indent_inc=8))))
    [
            1,
            2,
            [
    ...
    >>> print(list2str(list_dumper(a,0,Options(format_=STR_D,indent_inc=8))))
    [
            1, 2, 
            [
                    'A', 'B', 'C'
    ...
    if options.format==REPR:
        ...
        if options.start:
            ...
            opt= options
        inb= ", "
        oopt= Options(other=opt,format_=STR,start=False)
    elif options.format==STR:
        opn= ["\n",_ispc(indent),br_opn]
        cls= ["\n",_ispc(indent),br_cls]
        if options.start:
    ...
    l= [opn,
        ljoin([inb],\
              [ldump(x[1],indent+x[0].indent_inc,x[0]) \
               for x in _first_scalar(_myzip(val,opt),oopt)],
              False
             ),\
        cls\
       ]
...
def dict_dumper(val,indent,options=default_options):
    ...
    >>> print(list2str(dict_dumper(a,0,Options(format_=STR))))
    {
        'A' : 
            1,
        'B' : 
    ...
    >>> print(list2str(dict_dumper(a,0,Options(format_=REPR))))
    { 'A' : 1, 'B' : { 'XX' : 12, 'YY' : 13 }, 'C' : 5, 'kjhkjhkh' : 12123 }

    Printed in "STR" format it looks like this:
    >>> print(list2str(dict_dumper(a,0,Options(format_=STR, indent_inc=8))))
    {
        'A'   : 
                1,
        'B'   : 
    ...
    >>> print(list2str(dict_dumper(a,0,Options(format_=STR_D, indent_inc=8))))
    {
        'A'   : 1,
        'B'   : 
                {
    ...
    >>> print(list2str(dict_dumper(a,0,Options(format_=STR_D, indent_inc=4,key_indent=12))))
    {
                'A' : 1,
                'B' : 
                    {
...
def replace_dumper(dumpername,func):
    """replaces the built-in dumpers.

    parameters:
    dumpername -- Name of the dumper
    ...
    """
    if not dumpername in _dumpers_dict:
        raise TypeError("unknown dumpername: %s" % dumpername)
    dumpers_index= _dumpers_dict[dumpername]
...
def dumpstr(val,indent=0,options=default_options):
    ...
    >>> print(dumpstr([1,2],options=Options(format_=REPR)))
    [ 1, 2 ]
    >>> print(dumpstr([1,2],options=Options(format_=STR)))
    [
        1,
        2
    ]
    >>> print(dumpstr([1,2],indent=4,options=Options(format_=STR)))
        [
            1,
            2
        ]
    ...
    >>> print(dumpstr(["A",1,[2,3],{"X":1,"Y":2}],options=Options(format_=REPR)))
    [ 'A', 1, [ 2, 3 ], { 'X' : 1, 'Y' : 2 } ]

    >>> print(dumpstr(["A",1,[2,3],{"X":1,"Y":2}],options=Options(format_=STR)))
    [
        'A',
        1,
        [
    ...
    >>> print(dumpstr(["A",1,[2,3],{"X":1,"Y":2}],options=Options(format_=STR_D)))
    [
        'A', 1, 
        [
            2, 3
...
def drepr(val,indent=0):
    ...
    [ 1, { 'A' : [ 2, 3 ] }, 'XY' ]
    """
    return dumpstr(val,indent=indent,options=Options(format_=REPR))
...
def dstr(val,indent=0):
    ...
    """
    return dumpstr(val,indent=indent,options=Options(format_=STR))
...
def dstrd(val,indent=0):
    ...
    """
    return dumpstr(val,indent=indent,options=Options(format_=STR_D))
...
def dump(val,indent=0,options=default_options):
    ...
    Here are some examples:
    >>> dump({"A":1,"B":[1,2],"C":{"X":10,"Y":11}},options=Options(format_=REPR))
    { 'A' : 1, 'B' : [ 1, 2 ], 'C' : { 'X' : 10, 'Y' : 11 } }
    >>> dump({"A":1,"B":[1,2],"C":{"X":10,"Y":11}},options=Options(format_=STR))
    {
        'A' : 
            1,
        'B' : 
    ...
    >>> dump({"A":1,"B":[1,2],"C":{"X":10,"Y":11}},options=Options(format_=STR_D))
    {
        'A' : 1,
        'B' : 
            [
...
def prepr(val,indent=0):
    ...
    [ 1, { 'A' : [ 2, 3 ] }, 'XY' ]
    """
    dump(val,indent=indent,options=Options(format_=REPR))
...
def pstr(val,indent=0):
    ...
    """
    dump(val,indent=indent,options=Options(format_=STR))
...
def pstrd(val,indent=0):
    ...
    """
    dump(val,indent=indent,options=Options(format_=STR_D))
