...
def ljoin(var,l,typecheck=True):
    ...
    asrt_list(l)
    if typecheck:
        if not isinstance(var, type(l[-1])):
...
def lprefixsuffix(prefix,suffix,l,typecheck=True):
    ...
    asrt_list(l)
    if typecheck:
        if not isinstance(prefix, type(l[-1])):
            raise TypeError("prefix-var and list are not type-compatible: %s <-> %s" %\
                            (str(type(prefix)),str(type(l[-1]))))
        if not isinstance(suffix, type(l[-1])):
            raise TypeError("suffix-var and list are not type-compatible: %s <-> %s" %\
                            (str(type(suffix)),str(type(l[-1]))))
    if len(l)<1:
        return l
    new= []
    for elm in l:
...
def l_prefix_suffix_join(prefix,suffix,join,l,typecheck=True):
    ...
    typecheck -- Do a very simple typecheck. when True, the
                 ...
                 The default is True.
    """
    return ljoin(join,  lprefixsuffix(prefix,suffix,l, typecheck), typecheck)
